...
async fn agent() -> &'static (helpers::TestAgent, helpers::fixtures::GraphFixture) {
    ...
    static AGENT: OnceCell<(helpers::TestAgent, helpers::fixtures::GraphFixture)> = OnceCell::const_new();
    AGENT.get_or_init(|| async {
        start_agent_mock_with_fixture().await
    }).await
}
...
async fn retrieval_direct_entity_alice() {
    ...
    assert!(strings.iter().any(|f| f.to_lowercase().contains("alice")),
...
async fn retrieval_direct_entity_acme() {
    ...
    assert!(strings.iter().any(|f| f.to_lowercase().contains("acme")),
...
async fn retrieval_direct_entity_dr_smith() {
    ...
    assert!(strings.iter().any(|f| f.to_lowercase().contains("smith")),
...
async fn retrieval_who_works_at_acme() {
    ...
    assert!(combined.contains("alice") || combined.contains("bob") || combined.contains("henry"),
...
async fn retrieval_where_does_alice_live() {
    ...
    let combined = strings.join(" ").to_lowercase();
    assert!(combined.contains("alice") || combined.contains("london"),
...
async fn retrieval_alice_family() {
    ...
    assert!(combined.contains("bob") || combined.contains("carol") || combined.contains("sarah"),
...
async fn retrieval_cross_domain_medical_and_financial() {
    ...
    let combined = strings.join(" ").to_lowercase();
    assert!(combined.contains("diabetes") || combined.contains("metformin") || combined.contains("principal"),
...
async fn retrieval_cross_domain_research_collaboration() {
    let (ag, _fix) = agent().await;
    let facts = query_facts(&ag.client, &ag.base_url, "machine learning research collaboration", 15).await;
    let strings = fact_strings(&facts);
    assert!(!strings.is_empty(), "should find some facts for broad query");
...
async fn retrieval_high_confidence_facts_preferred() {
    ...
    if facts.len() >= 2 {
        ...
        assert!(conf_0 > 0.3, "top result should have decent confidence: {conf_0}");
    }
}
...
async fn retrieval_memory_tier_filter() {
    ...
    for fact in &facts {
        ...
        assert!(tier == "long_term" || tier.is_empty(),
...
async fn retrieval_nonexistent_entity() {
    ...
    let combined = strings.join(" ").to_lowercase();
    assert!(!combined.contains("zebediah"), "should not hallucinate nonexistent entity");
}
...
async fn retrieval_graph_stats() {
    ...
    assert!(active_edges.len() >= fix.edge_count() - 5,
