...
//!
//! `GraphClient` is the FalkorDB-specific implementation of `GraphBackend`.
//! Cypher-string helpers and `FalkorValue` extractors live in submodules.
...
use cypher::{approx_days_clause, sanitise, strip_tz, vec_literal, STOP_WORDS};
use values::{
    edge_row_from_values, extract_float, extract_int, extract_string, node_to_entity_row,
    supersession_from_row, take_n,
};
...
pub struct GraphClient {
    pub(crate) pool: Arc<Vec<Mutex<AsyncGraph>>>,
    pub(crate) next: Arc<std::sync::atomic::AtomicUsize>,
...
impl GraphClient {
    ...
    pub async fn promote_working_memory(&self) -> Result<usize> {
        ...
        let one_hour_ago = strip_tz(&one_hour_ago_rfc);
        let query = format!(
            "\
            MATCH ()-[r:RELATION]-() \
            WHERE r.invalid_at IS NULL \
    ...
    pub async fn decay_stale_edges(
        ...
        let now_rfc = now.to_rfc3339();
        let now_no_tz = strip_tz(&now_rfc);
        let days_stale_expr = approx_days_clause("accessed_dt", "now_dt", "days_stale");
        let query = format!(
            "MATCH ()-[r:RELATION]-() \
             WHERE r.invalid_at IS NULL \
