...
- **Concurrency control.** Each entity carries a `last_visited` timestamp. Queries filter out recently-visited entities; revisit window is configurable. Two parallel workers may briefly race on the same entity; the second one's update is a no-op because the first will have written the recency timestamp.
- **Selection strategy.** Weighted sampling with a tunable temperature. Score each candidate by priority (salience, age, contradiction signal, etc.); sample with weight `exp(score / temperature)`. Temperature near zero behaves like best-first; high temperature behaves like uniform random. One knob covers the spectrum. (Power-law sampling is an equivalent shape if preferred — both expose a single "exploitation vs exploration" knob.)
- **Termination.** Continuous in production. For `hippo.dream()` manual triggers, bounded by units-processed or tokens-consumed, whichever comes first. The "dream until idempotent" idea remains an open design: detecting "no effect" is non-trivial when the Dreamer is doing many different actions per visit. Provisional plan: budget-bounded passes; revisit when we have more data on real Dreamer behaviour.
